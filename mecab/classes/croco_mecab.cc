...
{
    php_croco_mecab_object *mecab_obj = Z_MECAB_P(ZEND_THIS);
    char *dictionary;
    ...
    reinterpret_cast<php_croco_mecab_object*>(Z_MECAB_P(ZEND_THIS))->handle = reinterpret_cast<TaggerHandle>(tagger);
}
/* }}} */
...
PHP_METHOD(croco_mecab_class, __destruct)
{
    ...
    php_croco_mecab_object *mecObj = Z_MECAB_P(ZEND_THIS);
    if (nullptr != mecObj->handle) {
        delete reinterpret_cast<MeCab::Tagger*>(mecObj->handle);
        mecObj->handle = nullptr;
    }
}
/* }}} */
...
PHP_METHOD(croco_mecab_class, parse)
{
    ...
    MeCab::Tagger *tagger = reinterpret_cast<MeCab::Tagger*>((Z_MECAB_P(ZEND_THIS))->handle);
