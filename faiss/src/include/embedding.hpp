...
public:
    Faiss() : _model(nullptr), _ctx(nullptr) {}
    ~Faiss() {
        if (_ctx) {
            llama_free(_ctx);
        }
    ...
    std::vector<std::vector<float>> getFaisss(const std::vector<std::string> &texts);
...
private:
    ...
    std::vector<float> _getFaiss(const llama_seq_id sequence_id);
}; // class Faiss
 ...
 * @return void
 */
inline void Faiss::loadModel(const char *model, const int32_t nThreads=4, uint32_t nUBatch=2048)
{
    ...
    ctx_params.faisss = true;               // faisss を有効化
 ...
 */
inline std::vector<std::vector<float>> Faiss::getFaisss(const std::vector<std::string> &texts)
{
    std::lock_guard<std::mutex> lock(_mutex);

    // 結果配列
    std::vector<std::vector<float>> faisss;
    ...
    while (current_idx < tokenList.size()) {
        ...
        for (size_t seq_idx = current_idx; seq_idx < batch_end; ++seq_idx) {
            llama_seq_id local_seq_id = static_cast<llama_seq_id>(seq_idx - current_idx);
            auto faiss = _getFaiss(local_seq_id);
            
            if (faiss.empty()) {
                std::cerr << "Warning: Empty faiss for sequence " << seq_idx << std::endl;
            }
            
            faisss.emplace_back(std::move(faiss));
        }
 ...
 */
inline std::vector<llama_token> Faiss::_tokenize(const std::string &text)
{
    // トークン化
 ...
 */
inline std::vector<float> Faiss::_getFaiss(const llama_seq_id sequence_id)
{
    // Faissを取得
    const int n_embd = llama_model_n_embd(_model);
    const float *raw_faiss = llama_get_faisss_seq(_ctx, sequence_id);
    
    if (!raw_faiss) {
        std::cerr << "Failed to get faisss" << std::endl;
        return std::vector<float>();
    }

    // 正規化（L2ノルム）
    std::vector<float> normalized_faiss(n_embd);
    float square_sum = 0.0f;
    for (int i = 0; i < n_embd; ++i) {
        square_sum += raw_faiss[i] * raw_faiss[i];
    }
    ...
    for (int i = 0; i < n_embd; ++i) {
        normalized_faiss[i] = raw_faiss[i] * normalize;
    }

    return normalized_faiss;
}
